    ...
    case ADD_OBJ_TO_LIST_OF_VISIBLE:
      if (action.value != undefined && action.value.val.length != 0) {
        ...
        if (filterState.visibleByProp['proc'].length > 0 ||
         ...
         filterState.visibleByProp['sh'].length > 0) { filterState.selectionBehaviour = 'on'; } else { filterState.selectionBehaviour = 'off'; }
...
export function applyFilter (data, filter) {
  ...
  data.process.childs = (data.process.childs || []).map(proc => {
    ...
    if (filter.selectionBehaviour === 'on') {
      ...
      if (!proc.visible) { proc.location.map(loc => { if (filter.visibleByProp.loc.includes(loc)) proc.visible = true; }); }
      if (!proc.visible) { proc.participants.map(sh => { if (filter.visibleByProp.sh.includes(sh)) proc.visible = true; }); }
    }
