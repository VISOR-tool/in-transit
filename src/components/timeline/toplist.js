...
class Toplist extends Component {
  render () {
    ...
    process.process.childs.map(p => {
      if (p.participation.includes('open') && new Date(p.start) > new Date()) { nextPrtcpDates.push([p.participation, p.start, p.name, p.id]); }
    });
    ...
    process.process.childs.map(p => {
      if (p.transformation.type === '>' && new Date(p.start) > new Date()) {
        if (p.transformation.info.length == 0) { nextDecisionDates.push([p.start, p.name + '(' + p.description + ')', p.id]); } else nextDecisionDates.push([p.start, p.transformation.info, p.id]);
      }
    });
    nextDecisionDates =
